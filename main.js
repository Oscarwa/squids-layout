...
const SQUID_DEFAULT_DURATION = 0.5; // minutes
...
const createSquid = (username, color) => {
  if (squids.length < maxSquids) {
    ...
    const acceleration = Math.floor(Math.random() * 2) + 1.5;
    const squid = {
      acc: acceleration,
      x,
      y,
...
if (debug !== null) {
  document.body.addEventListener("click", (e) => {
    e.preventDefault();
    // createSquid(`testing${squids.length}`, "#613EB4");
    commandMessage(
      `testing # ${Math.floor(Math.random() * 20)}`,
